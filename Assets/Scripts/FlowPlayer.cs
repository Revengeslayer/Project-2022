...
public class FlowPlayer : MonoBehaviour
{
    ...
    public Vector3 offect = new Vector3(16f, 10f, 0);
    ...
    private void Start()
    {
        ...
        gameObject.transform.forward = new Vector3(gameObject.transform.forward.x , gameObject.transform.forward.y, gameObject.transform.forward.z );
        CMRotate = gameObject.transform.forward;
        FollowVec = playerPos.position - gameObject.transform.position;
        FOV = Camera.main.fieldOfView;
        CameraDistance = (transform.position - playerPos.position).magnitude;
        rotationEuler = transform.rotation;
    }
    void Update()
    {
        //SetCameraPos();
    ...
    void ViewRotate()
    {
        if (Input.GetMouseButton(1))
        {
            ...
            transform.position = rotationEuler * new Vector3(0, 0, -CameraDistance) + playerPos.position;

            offect = transform.position - playerPos.position;

            //transform.forward = new Vector3(-offect.x, 0, -offect.z);
        }
    }
    //#region Set & Ray
    //void CameraRaycast()
